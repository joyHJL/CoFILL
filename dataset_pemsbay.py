...
#ob_mask是本身缺失的掩码，eval_mask是要验证的值就是选中去验证的值，gt_mask是再一次掩码后的掩码
class PemsBAY_Dataset(Dataset):
    def __init__(self, eval_length=24, mode="train", val_len=0.1, test_len=0.2, missing_pattern='block',
        ...
        ob_mask = (df.values != 0.).astype('uint8') #ob_mask是数据本身就缺失，没有真实值
        SEED = 9101112
    ...
    def __getitem__(self, org_index):
        ...
        if self.is_interpolate:
            ...
            s["coeffs"] = None  # 增加一个占位符，当is_interpolate为False时
        return s
